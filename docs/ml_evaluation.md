...
**What a generalization-honest evaluation requires.** A sample of real SMS (the `real` rows in the corpus are a natural candidate) must be hand-labeled by a human labeler blind to the rule system. The meaningful generalization metric is the model's agreement with human labels, not with auto-generated labels. A second labeler would additionally quantify label noise. Both are flagged as outstanding items in `docs/improvements.md`.
